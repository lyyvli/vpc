...
|Description|String|否|VBR|VBR的描述信息。长度为2-256个字符，必须以字母或中文开头，但不能以`http://`或`https://`开头。
 ...
 |
|DetectMultiplier|Long|否|3|检测时间倍数。即接收方允许发送方发送报文的最大连接丢包数，用来检测链路是否正常，取值：3-50。

 |
|LocalGatewayIp|String|否|116.62.xx.xx|VBR的阿里云侧互联IP。

 |
|MinRxInterval|Long|否|100|配置BFD报文的接收间隔，取值：100-1000，单位为us。

 |
|MinTxInterval|Long|否|100|配置BFD报文的发送间隔，取值：100-1000，单位为us。
