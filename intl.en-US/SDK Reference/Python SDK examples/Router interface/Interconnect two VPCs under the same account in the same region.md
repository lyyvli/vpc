# Interconnect two VPCs under the same account in the same region {#task_222315 .task}

This topic describes how to use the Python SDK to interconnect two VPCs under the same account in the same region.

Two VPCs are created under the same account in the same region.

Before you interconnect two VPCs, you must create a router interface for each VPC. When you create router interfaces, note the following limits:
...
-   If any overdue router interface exists under your account, you can no longer create router interfaces.
...
2.  Open the consts.py file in your text editor and set the ACS\_CLIENT parameter for authentication.
3.  In the SDK directory, open the $aliyun-openapi-python-sdk-examples\\sdk\_examples\\examples\\ec folder.
4.  Open the ec\_same\_account.py file in your text editor, set the parameters as needed, and then save the settings and exit the editor. 
    ...
    class RouterInterface(object):
        ...
        def create_router_interface(self, params):
            ...
            try:
                ...
                # Connect the ID of the region to which the acceptor belongs.
        ...
        def connect_router_interface(self, params):
            """
            connect_router_interface: The initiator router interface initiates a connection to the acceptor.
        ...
        def modify_router_interface_attribute(self, params):
            """
            modify_router_interface_attribute: Modify the configuration of the router interface.
        ...
        def describe_router_interface(self, instance_id):
            """
            describe_router_interface: Query the router interfaces in the specified region.
        ...
        def describe_ri_status(self, instance_id):
            """
            describe_ri_status: Query the status of the router interface in the specified region.
            API reference link: https://partners-intl.aliyun.com/help/doc-detail/36035.htm
            """
...
5.  Enter the directory where the ec\_same\_account.py file is stored, and then run the following command: 
